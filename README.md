...
长时间写代码、读文档或深度工作时，人容易沉入「时间感模糊」的状态：呼吸变浅、身体发僵而不自知。这里想做的是**弱打断、偏环境式**的提醒——不是番茄钟弹窗，而是像视野边缘一点缓慢的明暗变化，让你偶尔回到「自己还在呼吸」的感觉。更完整的产品与设计讨论见 [`win/docs/设计方案.md`](win/docs/设计方案.md)、[`win/docs/设计方案-补充.md`](win/docs/设计方案-补充.md)。

## 功能一览

- **氛围呼吸层**：透明置顶、不抢焦点、点击穿透；呼吸与色调会参考屏幕附近颜色做自适应（ambient 方向）。
- **活动与节奏推断**：在本地结合空闲时间、键盘节奏、鼠标移动与前台窗口停留等，粗略区分偏轻 / 偏沉浸等工作状态；**不上传**任何数据到网络。
- **轻柔文字提示**：在判断为长时间深度专注后，可能在氛围层上**偶尔**叠一两句很轻的短句（轮换文案、带冷却），用于回到呼吸与身体觉察。
...
以下为桌面呼吸动画效果的简短录屏；程序里另有上述文字提示，录屏里未拍到出现瞬间。
